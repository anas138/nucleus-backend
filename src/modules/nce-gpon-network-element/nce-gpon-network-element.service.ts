...
@Injectable()
export class NceGponNetworkElementService extends BaseService<NceGponNetworkElement> {
  constructor(
    private readonly repo: NceGponNetworkElementRepository,
  ...
  async syncGponNetworkElements(): Promise<any> {
    ...
    const resp = await this.listenerApiService.get(
      'nce-gpon-nms/network-elements',
    );
    ...
    try {
      await Promise.all(
        data.map(async (item) => {
          const networkElement = transformGponNetworkElementResponse(item);
          // map region against device name to be done
          networkElement.region_id =
            (await this.extractRegions(networkElement.name)) ?? null;
