...
@Controller('nce-gpon-network-element')
@UseGuards(AuthGuard(), RolePermissionGuard)
export class NceGponNetworkElementController {
  constructor(
    private nceGponNetworkElementService: NceGponNetworkElementService,
  ) {}

  @Get()
  @PermissionsMetadata(APP_PERMISSIONS.PUBLIC)
  async getAllNceGponNetworkElements(): Promise<INceGponNetworkElementModel[]> {
